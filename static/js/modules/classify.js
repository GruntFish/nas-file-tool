...
const ClassifyModule = {
    ...
    async preview() {
        ...
        try {
            ...
            if (result.results && result.results.length > 0) {
                ...
                previewList.appendChild(baseInfo);
                ...
                stats.textContent = '📊 共 ' + result.results.length + ' 个文件将分类到 ' +
                    Object.keys(grouped).length + ' 个目录' + (copyMode ? ' (复制模式)' : ' (移动模式)');
    ...
    async execute() {
        ...
        const actionText = copyMode ? '复制' : '移动';
        if (!confirm(`确定要${actionText}分类 ${files.length} 个文件吗？\n\n目标目录: ${window.currentPath || '/'}/${targetBase}`)) {
            return;
        }
